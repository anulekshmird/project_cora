...
class CopilotController(QThread):
    ...
    def process_visual_payload(self, payload):
        ...
        # FIX 1: Slide-aware signature generation
        # Include OCR hash and Window title to ensure unique per slide
        ocr_text = getattr(self.observer, 'last_ocr_text', '')
        import hashlib
        ocr_hash = hashlib.md5(ocr_text.encode()).hexdigest() if ocr_text else "no-ocr"
        win_title = (self.last_active_window or "unknown").lower()
        p_type = payload.get('type', 'visual')
        
        sig = f"{ocr_hash}:{win_title}:{p_type}"
    ...
    def start_proactive_loop(self):
        ...
        while self.running:
            try:
                ...
                if current_ocr != self.last_ocr_text_cache:
                    # If OCR changed significantly, reset dismissal history
                    # This allows the orb to reappear when moving to a new slide
                    if len(current_ocr) > 0 and current_ocr != self.last_ocr_text_cache:
                        print("Copilot: 📄 Context change detected (Slide/Page transition). Resetting dismissal history.")
                        self.dismissed_signatures.clear()
                    self.last_ocr_text_cache = current_ocr
                ...
                # ---------------------------------------------------------
                # E. FALLBACK: Visual / Maintenance (Restore if/elif/else chain)
                # ---------------------------------------------------------
                else:
